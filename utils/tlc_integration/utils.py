...
def get_or_create_tlc_table(input_url, split, revision_url='', root_url=None):
    ...
    if revision_url:
        ...
        table = table.latest(wait_for_rescan=True, timeout=10)

    table.ensure_fully_defined()
    return table


def tlc_table_to_yolo_labels(table: TableFromYolo):
    unpack_box = lambda bbox: [bbox[LABEL], bbox[X0], bbox[Y0], bbox[X1], bbox[Y1]]

    output_labels = []
    for row in table.table_rows:
        unpacked = [unpack_box(box) for box in row[BOUNDING_BOXES][BOUNDING_BOX_LIST]]
        arr = np.array(unpacked, ndmin=2)
        if len(unpacked) == 0:
            arr = arr.reshape(0, 5)
        output_labels.append(arr)
