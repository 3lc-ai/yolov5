...
from .utils import DatasetMode, tlc_table_to_yolo_labels
...
class TLCLoadImagesAndLabels(LoadImagesAndLabels):
    ...
    def __init__(
        ...
        self.sampling_weights = np.array([row[SAMPLE_WEIGHT] for row in table.table_rows])

        # Update dataset files and labels with 3LC values (this is the round-trip)
        n = len(table)
        self.im_files = [Url(row[IMAGE]).to_absolute().to_str() for row in table.table_rows]  # Unalias the image paths

        self.shapes = np.array([(row[WIDTH], row[HEIGHT]) for row in table.table_rows])
        ...
        self.labels = tlc_table_to_yolo_labels(table).copy()
        ...
        if num_fixed > 0 or num_corrupt > 0:
            LOGGER.info(f'Fixed {num_fixed} images and found {num_corrupt} corrupt images')
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size and self.dataset_mode == DatasetMode.TRAIN:  # normalized xywh to pixel xyxy format
        ...
        if nl and self.dataset_mode == DatasetMode.TRAIN:
...
def fix_image(im_file) -> tuple(bool, bool, str):
    fixed = False
    corrupt = False
    msg = ""
