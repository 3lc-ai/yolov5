...
| `TLC_SAMPLING_WEIGHTS=true/false`   | `false`     | Whether to use sampling weights in training. Zero-weighted samples are ignored completely, reducing the number of samples trained on per epoch to only the number of samples with nonzero weights. | `train.py`              |
| `TLC_IGNORE_ZERO_WEIGHT`            | `false`     | Whether to ignore zero-weighted samples in metrics collection.                                                                                                                                     | `train.py`              |
